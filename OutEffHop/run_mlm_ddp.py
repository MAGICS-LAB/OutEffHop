...
import math
import os
os.environ["WANDB_PROJECT"] = "outlier_free"
import random
import warnings
import numpy as np
...
def main():
    ...
    accelerator = Accelerator(
        gradient_accumulation_steps=args.gradient_accumulation_steps, mixed_precision="fp16", **accelerator_log_kwargs
    )
    ...
    num_layers = len(model.module.bert.encoder.layer)
    ffn_inf_norm = None
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if args.with_tracking and completed_steps % args.tb_scalar_log_interval == 0:
                ...
                for layer_idx in range(len(model.module.bert.encoder.layer)):
                    self_attn_layer = model.module.bert.encoder.layer[layer_idx].attention.self
        ...
        act_dict2 = {}
        if EXTRA_METRICS:
            act_dict2 = attach_act_hooks(model)
        ...
        # if args.is_ddp:
        #     outputs = [b.ffn.output for b in model.module.transformer.encoder.layer]
        #     attn_probs = [b.attn.attention_probs for b in model.module.transformer.encoder.layer]
        # else:
        #     outputs = [b.ffn.output for b in model.transformer.encoder.layer]
        #     attn_probs = [b.attn.attention_probs for b in model.transformer.encoder.layer]
        
        # wandb.log({"Inf_norm_output": max([o.abs().max() for o in outputs])}, step=completed_steps)
        # wandb.log({"kurtosis_output": kurtosis(torch.cat(outputs).detach().flatten().float().cpu().numpy())}, step=completed_steps)  
        # wandb.log({"kurtosis_output_mean": torch.mean(torch.tensor([kurtosis(o.detach().flatten().float().cpu().numpy()) for o in outputs]))}, step=completed_steps)
        try:
        ...
        if EXTRA_METRICS:
            ...
            accelerator.log({"max LN(FFN i + o) inf norm": max_LN_out_inf_norm}, step=completed_steps)
            accelerator.log({"Avg Kurtosis": avg_kurtosis}, step=completed_steps)  
            accelerator.log({"Max Kurtosis": max_kurtosis}, step=completed_steps)
            accelerator.log({"max FFN input + output inf norm": max_LN_inp_inf_norm},step=completed_steps)
            accelerator.log({"max FFN output inf norm":max_ffn_out_inf_norm}, step=completed_steps)
