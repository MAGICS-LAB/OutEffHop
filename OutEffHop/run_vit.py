...
_logger = logging.getLogger('run_vit')
...
group.add_argument('--report_to', default='wandb', type=str,
                   help='reporting destination (default: "wandb"')
group.add_argument('--with_tracking', action='store_true', default=False,
                   help='enable wandb tracking')
group.add_argument('--attn_softmax', default='vanilla', type=str)
group.add_argument('--run_name', default='', type=str)

group.add_argument("--fine_tuning", action="store_true")
group.add_argument(
    "--skip_attn",
    action="store_true",
...
def main():
    ...
    if args.with_tracking:
        ...
        accelerator_log_kwargs["project_dir"] = args.output
    
    wandb.init(
        name=args.run_name,
        project = 'outlier_free',
    )
    ...
    if args.pretrained is not False and args.attn_gate_type != "none":
        state_dict = torch.load(str(Path(args.pretrained) / "pytorch_model.bin"))
        new_state_dict = {}
    ...
    class SubsetDataset(Dataset):
        def __init__(self, existing_dataset, percent_to_select=0.10):
            self.existing_dataset = existing_dataset
            self.percent_to_select = percent_to_select

            # Determine the number of samples to select
            num_samples = len(existing_dataset)
            num_samples_to_select = int(percent_to_select * num_samples)

            # Create a random sampler to select the desired percentage of data
            indices = list(range(num_samples))
            np.random.shuffle(indices)
            self.selected_indices = indices[:num_samples_to_select]

        def __len__(self):
            return len(self.selected_indices)

        def __getitem__(self, idx):
            # Retrieve the selected samples from the existing dataset
            selected_idx = self.selected_indices[idx]
            return self.existing_dataset[selected_idx]
        
        
    #dataset_train = SubsetDataset(dataset_train, percent_to_select=0.10)
    ...
    #dataset_eval = SubsetDataset(dataset_eval, percent_to_select=0.10)
    # setup mixup / cutmix
    collate_fn = None
    mixup_fn = None
    ...
    try:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            train_metrics, completed_steps, act_dict = train_one_epoch(
                epoch,
                model,
                loader_train,
                optimizer,
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if args.with_tracking and completed_steps % args.tb_scalar_log_interval == 0:
            ...
            for name, x in act_dict.items():
                ...
                wandb.log({f"{name}.act_inf_norm": x_inf_norm}, step=completed_steps)

            # gate probs (if present)
            for layer_idx in range(len(model.module.blocks)):
                self_attn_layer = model.module.blocks[layer_idx].attn
    ...
    return OrderedDict([('loss', losses_m.avg)]), completed_steps, act_dict
